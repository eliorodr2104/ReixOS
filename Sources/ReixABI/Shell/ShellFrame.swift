...
public struct ShellFrame {
    ...
    private init(_ envelope: ShellEnvelope, bytes: UnsafePointer<UInt8>, length: Int) {
        self.envelope = envelope
        self.bytes = bytes
        self.length = length
    }

    public static func decode(_ bytes: UnsafePointer<UInt8>, length: Int) -> ShellFrame? {
        ...
        let (total, overflow) = ShellProtocol.headerBytes.addingReportingOverflow(Int(envelope.payloadLength))
