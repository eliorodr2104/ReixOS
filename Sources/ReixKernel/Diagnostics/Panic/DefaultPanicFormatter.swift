...
public struct DefaultPanicFormatter: PanicFormatter, Loggable {
    ...
    /// How close both kernel stacks came to their limits.
    ///
    /// This is what separates "the stack overflowed" from "the deepest this
    /// kernel had ever gone was 7 KiB, and then one call ate the other 57":
    /// the first suspects a slow leak, the second names a single frame. On a
    /// guard-page panic the kernel figure reads as its full capacity, and the
    /// exception stack's own number says how much of the report path is left.
    private static func emitStacks() {
        PanicConsole.field("Stacks:")

        emitStack(StackUsage.kernelStack,    of: StackUsage.kernelStackCapacity)
        PanicConsole.write(" kernel   ")

        emitStack(StackUsage.exceptionStack, of: StackUsage.exceptionStackCapacity)
        PanicConsole.write(" exception")

        PanicConsole.newline()
    }

    private static func emitStack(_ used: UInt64, of capacity: UInt64) {
        PanicConsole.writeDec(used)
        PanicConsole.write("/")
        PanicConsole.writeDec(capacity)
