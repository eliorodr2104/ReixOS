...
public struct ShellLineEditor {
    ...
    /// Leaves room for the UTF-8 prompt in an atomic replace-line patch.
    public static let capacity        = 248
    ...
    private var pasteSnapshot         = InlineArray<256, UInt8>(repeating: 0)
    private var pasteSnapshotCount    = 0
    private var pasteSnapshotCursor   = 0
    ...
    public mutating func apply(
        ...
        let kind: ReixInputKind
        if event.kind == .key {

            guard event.phase != .release,
                  let keyKind = inputKind(for: event.logicalKey)
            else { return refused() }
        ...
        if pasteActive {
            ...
            switch kind {
                ...
                case .pasteChunk:
                    guard event.count > 0,
                          event.count <= Self.capacity - count
                    else { return rollbackPaste() }
                ...
                case .pasteBegin, .cancel, .focusLost, .stateReset:
                    return rollbackPaste()
        ...
        if event.kind == .key,
           event.phase == .repeatKey {

            guard repeatable(kind) else { return refused() }
        ...
        switch kind {
            ...
            case .insert, .textChunk, .compositionCommit:
            ...
            case .pasteBegin:
                beginPaste()
                return ShellEditorUpdate(action: .editing, patch: nil)

            case .pasteChunk, .pasteEnd:
                return refused()
            ...
            case .enter:
                ...
                if case .incomplete(let indent) = completeness {
                    ...
                    while index > cursor {
                        index -= 1
                        bytes[index + added] = bytes[index]
                    }

                    bytes[cursor] = 0x0A

                    if added > 1 {
                        for offset in 1..<added { bytes[cursor + offset] = 0x20 }
                    }
                ...
                return ShellEditorUpdate(
                    action: .submitted(count),
                    patch : ReixTextSurfaceCommand(kind: .newline, sequence: patchSequence)
                )
            ...
            case .resize:
                return ShellEditorUpdate(
                    action: .resized(event.width, event.height),
                    patch : nil
                )
            ...
            case .compositionBegin, .compositionUpdate, .compositionCancel,
                 .focusGained, .focusLost, .stateReset, .key:
                return refused()
        }
    ...
    @inline(__always)
    public func copyLine(
        into destination: inout InlineArray<256, UInt8>
    ) -> Int {
    ...
    private func inputKind(for key: ReixInputKey) -> ReixInputKind? {
        switch key {
            case .left                                   : .left
            case .right                                  : .right
            case .up                                     : .up
            ...
            case .cancel                                 : .cancel
            case .eof                                    : .eof
            case .historyPrevious                        : .historyPrevious
            case .historyNext                            : .historyNext
            case .none, .tab, .escape, .pageUp, .pageDown: nil
        }
    }

    private mutating func beginPaste() {

        pasteSnapshotCount  = count
        pasteSnapshotCursor = cursor

        for index in 0..<count { pasteSnapshot[index] = bytes[index] }

        pasteActive = true
    }

    @inline(__always)
    private func repeatable(_ kind: ReixInputKind) -> Bool {
        switch kind {
            case .left, .right, .home, .end, .backspace, .delete:
                true
    ...
    private mutating func applyRepeats(
    ...
    private mutating func applyRepeat(_ kind: ReixInputKind) -> Bool {

        switch kind {

            case .left:
                guard cursor > 0 else { return false }
                cursor = previousBoundary(before: cursor)
                return true

            case .right:
                guard cursor < count else { return false }
                cursor = nextBoundary(after: cursor)
                return true
            ...
            case .backspace:
                guard cursor > 0 else { return false }
                let start = previousBoundary(before: cursor)
                remove(at: start, count: cursor - start)
                cursor = start
                return true
    ...
    private mutating func rollbackPaste() -> ShellEditorUpdate {

        count  = pasteSnapshotCount
        cursor = pasteSnapshotCursor

        for index in 0..<count { bytes[index] = pasteSnapshot[index] }
    ...
    private mutating func insert(_ event: ReixInputRecord) {
        ...
        for offset in 0..<event.count {
            bytes[cursor + offset] = event.text[offset]
        }
    ...
    private mutating func clear(
        ...
        cursor              = 0
        pasteActive         = false
        pasteSnapshotCount  = 0
        pasteSnapshotCursor = 0
    ...
    private mutating func replaced(
        ...
        for index in 0..<Self.promptBytes {
            row[index] = Self.prompt.utf8Start[index]
        }

        for index in 0..<count {
            row[Self.promptBytes + index] = bytes[index]
        }

        let rowCount      = Self.promptBytes + count
        ...
        let update = row.span.withUnsafeBufferPointer {
            ...
            ShellEditorUpdate(
                ...
                patch : ReixTextSurfaceCommand(
                    ...
                    cursorColumn     : UInt16(logicalColumn + (
                        newRow == 0 ? Self.promptColumns : 0)
                    )
                )
            )
        }
