...
//
// Every artifact lands in `.reix/` at the package root — a command plugin can
// only write inside the package directory, and a single hidden folder keeps the
// root clean. The initrd is packed with `-C` on that folder so the tar entries
// stay bare names (`Init.elf`), which is what the kernel's TarFileSystem looks
// up at spawn time.
//
...
struct ReixPlugin: CommandPlugin {
    ...
    func performCommand(context: PluginContext, arguments: [String]) async throws {
        ...
        try run(tarTool, ["-cf", initrd.path,
                          "-C", out.path] + apps.map { "\($0).elf" }, cwd: root)
