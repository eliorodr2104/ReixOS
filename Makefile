...
# Pinned to the swiftly toolchain (6.3.x ships the Embedded stdlib for the
# bare-metal triple). A plain `swift` on PATH may resolve to Xcode's toolchain,
# which fails with "unable to load standard library for target aarch64-none-none-elf".
# Override on the command line if your toolchain lives elsewhere: make SWIFT=...
SWIFT       ?= /Users/mac/.swiftly/bin/swift
